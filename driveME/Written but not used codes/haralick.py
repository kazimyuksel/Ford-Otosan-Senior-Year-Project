# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 10:59:51 2019
...
segments = watershed(gradient, markers=250, compactness=0.0005)
#segments = slic(img_smooth, n_segments=250, compactness=10, sigma=1.0)
threshold = np.zeros((new_width,new_height),dtype=np.uint8)
(means, stds) = cv2.meanStdDev(hsv[250:270,250:270,:])
means[2,0] = means[2,0]
means[1,0] = means[1,0]
means[0,0] = means[0,0]*0.1
stds[0,0] = stds[0,0]*0.1
features_M = mahotas.features.haralick(hsv[250:270,250:270,:]).mean(axis=0)
features_M = features_M*1.0
features = np.concatenate([means, stds]).flatten()
features = np.concatenate([features, features_M])
for i in range(len(np.unique(segments))):
    ...
    means[0,0] = means[0,0]*0.1
    stds[0,0] = stds[0,0]*0.1
    res = cv2.bitwise_and(hsv_small,hsv_small,mask = mask_label)
    label_features_M = mahotas.features.haralick(res).mean(axis=0)
    #print(label_features_M)
    label_features_M = label_features_M*1.0
    label_feature = np.concatenate([means, stds]).flatten()
    label_feature = np.concatenate([label_feature, label_features_M])
...
cv2.imshow('occupancy_thres', res)
cv2.waitKey(0)
